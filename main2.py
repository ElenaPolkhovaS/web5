...
async def get_currency_rates(days, currencies=None):
    ...
    async with aiohttp.ClientSession() as session:
        ...
        for i, date in enumerate(results):
            ...
            for currency in date['exchangeRate']:
                if not currencies or currency['currency'] in currencies:
...
def parse_arguments():
    ...
    parser.add_argument('--currencies', nargs='+', help='Currencies to include in the result')
...
def main():
    ...
    result = loop.run_until_complete(get_currency_rates(args.days, args.currencies))
    for entry in result:
        print(entry)
